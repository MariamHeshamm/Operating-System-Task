...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	unsigned int env_page_ws_get_size_workingset = env_page_ws_get_size(curenv);
	unsigned int max_size_of_ws = curenv->page_WS_max_size;
	bool try = 0;
	if(env_page_ws_get_size_workingset < max_size_of_ws)
	{
		//hn3ml placement
		unsigned int *ptr_page_table = NULL;
	    struct Frame_Info *frame = NULL;
		int ret1 = allocate_frame(&frame);
		ret1 = map_frame(curenv->env_page_directory, frame, (void*)fault_va, PERM_WRITEABLE|PERM_USER);
		tlbflush();
		int ret2 = pf_read_env_page(curenv,(void*)fault_va);
		if(ret2 == E_PAGE_NOT_EXIST_IN_PF)
		{
			if(fault_va >= USTACKBOTTOM/*USER_HEAP_MAX*/ && fault_va < USTACKTOP)
			{
				int ret3 = pf_add_empty_env_page(curenv, fault_va, 1);
				if(ret3 == E_NO_PAGE_FILE_SPACE){
		...
		for (int i = 0;i < max_size_of_ws; i++)
		{
			unsigned int env_page_ws_get_size_workingset = env_page_ws_get_size(curenv);
			if(curenv->ptr_pageWorkingSet[i].empty == 1)
			{
				curenv->ptr_pageWorkingSet[i].empty = 0;
				curenv->ptr_pageWorkingSet[i].virtual_address = fault_va;
				break;
			}
		}

		/*curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address = ROUNDDOWN(fault_va,PAGE_SIZE);
	...
	{
		// N maxsweeps
		//counter in struct working set
		unsigned int address_elvictim_b2a = -1;
		int index = -1;

		for (int i = 0; i < max_size_of_ws; i++)
		{
		//	cprintf("D5LT FOR\n");
		unsigned int curva = curenv->ptr_pageWorkingSet[i].virtual_address;
		unsigned int *ptr_page_table1 = NULL;
	    struct Frame_Info *frame1 = get_frame_info(curenv->env_page_directory, (void *)curva,&ptr_page_table1);
		uint32 PERMUSED = ptr_page_table1[PTX(curva)]&PERM_USED;
		if(PERMUSED!=0)
		{
			ptr_page_table1[PTX(curva)]&=(~PERM_USED);
			curenv->ptr_pageWorkingSet[i].counter = 0;
			/*if (page_WS_max_sweeps == curenv->ptr_pageWorkingSet[curva].counter)
			{
				address_elvictim_b2a = curva;
				break;
			}*/
		...
		{
			if(curenv->ptr_pageWorkingSet[i].counter < page_WS_max_sweeps)
			{
		       curenv->ptr_pageWorkingSet[i].counter++;
			}
           if (page_WS_max_sweeps == curenv->ptr_pageWorkingSet[i].counter)
           {
        	   if(try == 0) //ana keda keda lw la2et address 5alas b7tfz beh bas lazem akml lfa kmla
        	   {
        		   address_elvictim_b2a = curva;
        		   index = i; //bshel elindex bta3o 3shan h7tago fl working set
        		   try = 1;
        	   }
        	   //break;
           }
		}
		if (address_elvictim_b2a == -1 && index == -1)
		{
			if(i == max_size_of_ws - 1)
				i = -1;
		}
		}

		//}
		unsigned int victimaddress = address_elvictim_b2a;

		unsigned int *ptr_page_table1 = NULL;
		struct Frame_Info *frame1 = get_frame_info(curenv->env_page_directory, (void *)victimaddress,&ptr_page_table1);
		uint32 PERMODIFIED = ptr_page_table1[PTX(victimaddress)]&PERM_MODIFIED;
		if(PERMODIFIED == 0)
		{
			 unmap_frame(curenv->env_page_directory,(void*)victimaddress);
		}
		else
		{
			int ret = pf_update_env_page(curenv, (void*)victimaddress, frame1);
			unmap_frame(curenv->env_page_directory,(void*)victimaddress);
		}
		...
		curenv->ptr_pageWorkingSet[index].virtual_address = ROUNDDOWN(fault_va,PAGE_SIZE);
		curenv->ptr_pageWorkingSet[index].empty = 0;
